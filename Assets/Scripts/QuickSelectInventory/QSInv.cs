...
public class QSInv : MonoBehaviour
{
    ...
    void SetItemSlots(int slots, Vector2[] pos)
    {
        ...
        {
            GUI.DrawTexture(new Rect(pos[i].x - (scrW * iconSizeNum * .5f), pos[i].y - (scrH * iconSizeNum * .5f), scrW * iconSizeNum, scrH * iconSizeNum), inv[i].Icon);
        }
    }
    ...
    void CalculateMouseAngles()
    {
        ...
        mouseIndex = CheckCurrentSector(mouseIndex);
        if (input.x != 0 || input.y != 0)
        {
    ...
    void OnGUI()
    {
        if (showSelectMenu)
        {
            ...
            //deadzone
            GUI.Box(new Rect(Scr(7.5f, 4), Scr(1, 1)), "Empty Content");
            //circle
            ...
            if (showBoxes)
            {
                for (int i = 0; i < numOfSectors; i++)
                {
                    GUI.DrawTexture(new Rect(slotPos[i].x - (scrW * iconSizeNum * .5f), slotPos[i].y - (scrH * iconSizeNum * .5f), scrW * iconSizeNum, scrH * iconSizeNum), slotTexture);
                }
            }
            if (showBounds)
            {
                for (int i = 0; i < numOfSectors; i++)
                {
                    GUI.Box(new Rect(boundPos[i].x - (scrW * 0.1f * .5f), boundPos[i].y - (scrH * 0.1f * .5f), scrW * 0.1f, scrH * 0.1f), "");
                }
            }
            if (showIcons)
            {
